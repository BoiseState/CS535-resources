...
public class RDDPlayground
{
	public static void main(String[] args)
	{
		...
		// List<Integer> set1 = IntStream.rangeClosed(1,
		// 3).boxed().collect(Collectors.toList());
		// List<Integer> set2 = IntStream.rangeClosed(2,
		// 3).boxed().collect(Collectors.toList());
...
//		});
		...
		JavaPairRDD<Object, Integer> set5 = set4.mapToPair(s -> new Tuple2<>(s, 1));
		set5 = set5.mapValues(r -> r + 1);
		printPairRDD("set5.mapValues(r -> r + 1);", set5);

		JavaPairRDD<Object, List<Integer>> set6 = set3.mapToPair(s -> new Tuple2<>(s, Arrays.asList(1, 2, 3)));
		rdd6.collect().forEach(s -> System.out.print(s + " "));
		System.out.println();

		JavaPairRDD<Object, Integer> set7 = set6.flatMapValues(s -> s.iterator());
	...
	private static void printPairRDD(String message, JavaPairRDD<?, Integer> rdd)
	{
		System.out.println(message);
		rdd.collect().forEach(x -> System.out.print(x + " "));
		System.out.println();
